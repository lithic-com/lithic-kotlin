...
interface BacktestService {
     ...
     */
    fun withOptions(modifier: (ClientOptions.Builder) -> Unit): BacktestService
     ...
     * Backtesting for `VELOCITY_LIMIT` rules is generally not supported. In specific cases (i.e.
     * where Lithic has pre-calculated the requested velocity metrics for historical transactions),
     * a backtest may be feasible. However, such cases are uncommon and customers should not
     * anticipate support for velocity backtests under most configurations. If a historical
     * transaction does not feature the required inputs to evaluate the rule, then it will not be
    ...
    interface WithRawResponse {
         ...
         * The original service is not modified.
         */
        fun withOptions(modifier: (ClientOptions.Builder) -> Unit): BacktestService.WithRawResponse
