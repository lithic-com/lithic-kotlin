...
private constructor(
     ...
     *   server responded with an unexpected value).
     */
    fun loanTapeRebuildConfiguration(): LoanTapeRebuildConfiguration? =
        loanTapeRebuildConfiguration.getNullable("loan_tape_rebuild_configuration")

    /**
     * @throws LithicInvalidDataException if the JSON field has an unexpected type (e.g. if the
     *   server responded with an unexpected value).
     */
    fun tierScheduleChangedAt(): OffsetDateTime? =
        tierScheduleChangedAt.getNullable("tier_schedule_changed_at")
    ...
    fun validate(): LoanTapeConfiguration = apply {
        ...
        updatedAt()
        creditProductToken()
        loanTapeRebuildConfiguration()?.validate()
        tierScheduleChangedAt()
        validated = true
    }
    ...
    internal fun validity(): Int =
        (if (createdAt.asKnown() == null) 0 else 1) +
            ...
            (if (instanceToken.asKnown() == null) 0 else 1) +
            (if (updatedAt.asKnown() == null) 0 else 1) +
            (if (creditProductToken.asKnown() == null) 0 else 1) +
            (loanTapeRebuildConfiguration.asKnown()?.validity() ?: 0) +
            (if (tierScheduleChangedAt.asKnown() == null) 0 else 1)
