 ...
 * The performance of the auth rule will be assessed on the configuration of the auth rule at the
 * time the report is requested. This implies that if a performance report is requested, right after
 * updating an auth rule, depending on the number of events processed for a card program, it may be
 * the case that no data is available for the report. Therefore Lithic recommends to decouple making
 * updates to an Auth Rule, and requesting performance reports.
 *
 * To make this concrete, consider the following example:
 * 1. At time `t`, a new Auth Rule is created, and applies to all auth events on a card program. The
 ...
 *    at this moment, it will only contain data for events to which both the active version and the
 *    draft version is applied. Lithic does this to ensure that performance reports represent a fair
 *    comparison between rules. Because there may be no events in this window, and because there may
 *    be some lag before data is available in a performance report, the requested performance report
 *    could contain no to little data.
 ...
 *    events to which both the current version of the Auth rule (in enforcing mode) and the draft
 *    version of the Auth rule (in shadow mode) applied.
 *
...
private constructor(
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is AuthRuleV2ReportParams &&
