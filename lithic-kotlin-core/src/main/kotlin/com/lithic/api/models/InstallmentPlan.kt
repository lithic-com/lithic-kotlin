...
private constructor(
     ...
     */
    fun closedAt(): LocalDate? = closedAt.getNullable("closed_at")
     ...
     *   server responded with an unexpected value).
     */
    fun sourceAmounts(): TransactionCategoryBalances? = sourceAmounts.getNullable("source_amounts")
    ...
    internal fun validity(): Int =
        (if (token.asKnown() == null) 0 else 1) +
            ...
            (if (feeAmount.asKnown() == null) 0 else 1) +
            (if (financialAccountToken.asKnown() == null) 0 else 1) +
            (if (installmentPlanTotal.asKnown() == null) 0 else 1) +
            (installments.asKnown()?.sumOf { it.validity().toInt() } ?: 0) +
            (if (installmentsOutstanding.asKnown() == null) 0 else 1) +
            (if (installmentsPaid.asKnown() == null) 0 else 1) +
            (if (numInstallments.asKnown() == null) 0 else 1) +
            (if (principalAmount.asKnown() == null) 0 else 1) +
            (sourceAmounts.asKnown()?.validity() ?: 0) +
            (if (sourceId.asKnown() == null) 0 else 1) +
            (sourceType.asKnown()?.validity() ?: 0) +
            (if (startDate.asKnown() == null) 0 else 1) +
            (state.asKnown()?.validity() ?: 0) +
            (if (totalPaid.asKnown() == null) 0 else 1) +
    ...
    private constructor(
         ...
         *   server responded with an unexpected value).
         */
        fun dateAssessed(): LocalDate? = dateAssessed.getNullable("date_assessed")
        ...
        internal fun validity(): Int =
            (if (amountDue.asKnown() == null) 0 else 1) +
                (amountDueDetails.asKnown()?.validity() ?: 0) +
                (if (amountOutstanding.asKnown() == null) 0 else 1) +
                (amountOutstandingDetails.asKnown()?.validity() ?: 0) +
                (if (amountPaid.asKnown() == null) 0 else 1) +
                (amountPaidDetails.asKnown()?.validity() ?: 0) +
                (if (dateAssessed.asKnown() == null) 0 else 1) +
                (if (dueDate.asKnown() == null) 0 else 1) +
                (if (installmentNum.asKnown() == null) 0 else 1) +
                (if (paymentDueDate.asKnown() == null) 0 else 1) +
                (payments.asKnown()?.sumOf { it.validity().toInt() } ?: 0)
        ...
        private constructor(
            ...
            internal fun validity(): Int =
                (if (amount.asKnown() == null) 0 else 1) +
                    (amountDetails.asKnown()?.validity() ?: 0) +
    ...
    private constructor(private val value: JsonField<String>) : Enum {
        ...
        companion object {
            ...
            val UNPAID_BALANCE = of("UNPAID_BALANCE")

            val TRANSACTION = of("TRANSACTION")

            fun of(value: String) = InstallmentPlanSource(JsonField.of(value))
        }
    ...
    private constructor(private val value: JsonField<String>) : Enum {
        ...
        companion object {
            ...
            val REBUILD_IN_PROGRESS = of("REBUILD_IN_PROGRESS")

            val FULLY_PAID = of("FULLY_PAID")

            val CANCELLED = of("CANCELLED")

            fun of(value: String) = InstallmentPlanState(JsonField.of(value))
        }
