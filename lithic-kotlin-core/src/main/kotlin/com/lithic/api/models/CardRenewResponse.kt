...
private constructor(
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [CardRenewResponse].
         *
    ...
    /** A builder for [CardRenewResponse]. */
    class Builder internal constructor() {
        ...
        internal fun from(cardRenewResponse: CardRenewResponse) = apply {
            token = cardRenewResponse.token
            accountToken = cardRenewResponse.accountToken
            cardProgramToken = cardRenewResponse.cardProgramToken
            created = cardRenewResponse.created
            funding = cardRenewResponse.funding
            lastFour = cardRenewResponse.lastFour
            pinStatus = cardRenewResponse.pinStatus
            spendLimit = cardRenewResponse.spendLimit
            spendLimitDuration = cardRenewResponse.spendLimitDuration
            state = cardRenewResponse.state
            type = cardRenewResponse.type
            authRuleTokens = cardRenewResponse.authRuleTokens.map { it.toMutableList() }
            cardholderCurrency = cardRenewResponse.cardholderCurrency
            cvv = cardRenewResponse.cvv
            digitalCardArtToken = cardRenewResponse.digitalCardArtToken
            expMonth = cardRenewResponse.expMonth
            expYear = cardRenewResponse.expYear
            hostname = cardRenewResponse.hostname
            memo = cardRenewResponse.memo
            pan = cardRenewResponse.pan
            pendingCommands = cardRenewResponse.pendingCommands.map { it.toMutableList() }
            productId = cardRenewResponse.productId
            replacementFor = cardRenewResponse.replacementFor
            additionalProperties = cardRenewResponse.additionalProperties.toMutableMap()
        }
        ...
        /**
         * Returns an immutable instance of [CardRenewResponse].
         *
    ...
    fun validate(): CardRenewResponse = apply {
        if (validated) {
            return@apply
        }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return /* spotless:off */ other is CardRenewResponse && token == other.token && accountToken == other.accountToken && cardProgramToken == other.cardProgramToken && created == other.created && funding == other.funding && lastFour == other.lastFour && pinStatus == other.pinStatus && spendLimit == other.spendLimit && spendLimitDuration == other.spendLimitDuration && state == other.state && type == other.type && authRuleTokens == other.authRuleTokens && cardholderCurrency == other.cardholderCurrency && cvv == other.cvv && digitalCardArtToken == other.digitalCardArtToken && expMonth == other.expMonth && expYear == other.expYear && hostname == other.hostname && memo == other.memo && pan == other.pan && pendingCommands == other.pendingCommands && productId == other.productId && replacementFor == other.replacementFor && additionalProperties == other.additionalProperties /* spotless:on */
    }
    ...
    override fun toString() =
        "CardRenewResponse{token=$token, accountToken=$accountToken, cardProgramToken=$cardProgramToken, created=$created, funding=$funding, lastFour=$lastFour, pinStatus=$pinStatus, spendLimit=$spendLimit, spendLimitDuration=$spendLimitDuration, state=$state, type=$type, authRuleTokens=$authRuleTokens, cardholderCurrency=$cardholderCurrency, cvv=$cvv, digitalCardArtToken=$digitalCardArtToken, expMonth=$expMonth, expYear=$expYear, hostname=$hostname, memo=$memo, pan=$pan, pendingCommands=$pendingCommands, productId=$productId, replacementFor=$replacementFor, additionalProperties=$additionalProperties}"
}
