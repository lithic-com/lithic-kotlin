...
private constructor(
    ...
    class Builder internal constructor() {
        ...
        internal fun from(cardRetrieveResponse: CardRetrieveResponse) = apply {
            token = cardRetrieveResponse.token
            accountToken = cardRetrieveResponse.accountToken
            cardProgramToken = cardRetrieveResponse.cardProgramToken
            created = cardRetrieveResponse.created
            funding = cardRetrieveResponse.funding
            lastFour = cardRetrieveResponse.lastFour
            pinStatus = cardRetrieveResponse.pinStatus
            spendLimit = cardRetrieveResponse.spendLimit
            spendLimitDuration = cardRetrieveResponse.spendLimitDuration
            state = cardRetrieveResponse.state
            type = cardRetrieveResponse.type
            authRuleTokens = cardRetrieveResponse.authRuleTokens.map { it.toMutableList() }
            cardholderCurrency = cardRetrieveResponse.cardholderCurrency
            cvv = cardRetrieveResponse.cvv
            digitalCardArtToken = cardRetrieveResponse.digitalCardArtToken
            expMonth = cardRetrieveResponse.expMonth
            expYear = cardRetrieveResponse.expYear
            hostname = cardRetrieveResponse.hostname
            memo = cardRetrieveResponse.memo
            pan = cardRetrieveResponse.pan
            pendingCommands = cardRetrieveResponse.pendingCommands.map { it.toMutableList() }
            productId = cardRetrieveResponse.productId
            replacementFor = cardRetrieveResponse.replacementFor
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return /* spotless:off */ other is CardRetrieveResponse && token == other.token && accountToken == other.accountToken && cardProgramToken == other.cardProgramToken && created == other.created && funding == other.funding && lastFour == other.lastFour && pinStatus == other.pinStatus && spendLimit == other.spendLimit && spendLimitDuration == other.spendLimitDuration && state == other.state && type == other.type && authRuleTokens == other.authRuleTokens && cardholderCurrency == other.cardholderCurrency && cvv == other.cvv && digitalCardArtToken == other.digitalCardArtToken && expMonth == other.expMonth && expYear == other.expYear && hostname == other.hostname && memo == other.memo && pan == other.pan && pendingCommands == other.pendingCommands && productId == other.productId && replacementFor == other.replacementFor && additionalProperties == other.additionalProperties /* spotless:on */
    }
    ...
    override fun toString() =
        "CardRetrieveResponse{token=$token, accountToken=$accountToken, cardProgramToken=$cardProgramToken, created=$created, funding=$funding, lastFour=$lastFour, pinStatus=$pinStatus, spendLimit=$spendLimit, spendLimitDuration=$spendLimitDuration, state=$state, type=$type, authRuleTokens=$authRuleTokens, cardholderCurrency=$cardholderCurrency, cvv=$cvv, digitalCardArtToken=$digitalCardArtToken, expMonth=$expMonth, expYear=$expYear, hostname=$hostname, memo=$memo, pan=$pan, pendingCommands=$pendingCommands, productId=$productId, replacementFor=$replacementFor, additionalProperties=$additionalProperties}"
}
