...
private constructor(
    ...
    /** A builder for [V2UpdateResponse]. */
    class Builder internal constructor() {
        ...
        internal fun from(v2UpdateResponse: V2UpdateResponse) = apply {
            ...
            accountTokens = v2UpdateResponse.accountTokens.map { it.toMutableList() }
            businessAccountTokens =
                v2UpdateResponse.businessAccountTokens.map { it.toMutableList() }
            cardTokens = v2UpdateResponse.cardTokens.map { it.toMutableList() }
            currentVersion = v2UpdateResponse.currentVersion
            draftVersion = v2UpdateResponse.draftVersion
            eventStream = v2UpdateResponse.eventStream
            lithicManaged = v2UpdateResponse.lithicManaged
            ...
            excludedCardTokens = v2UpdateResponse.excludedCardTokens.map { it.toMutableList() }
    ...
    override fun toString() =
        "V2UpdateResponse{token=$token, accountTokens=$accountTokens, businessAccountTokens=$businessAccountTokens, cardTokens=$cardTokens, currentVersion=$currentVersion, draftVersion=$draftVersion, eventStream=$eventStream, lithicManaged=$lithicManaged, name=$name, programLevel=$programLevel, state=$state, type=$type, excludedCardTokens=$excludedCardTokens, additionalProperties=$additionalProperties}"
}
