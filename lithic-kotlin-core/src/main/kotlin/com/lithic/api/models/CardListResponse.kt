...
private constructor(
    ...
    class Builder internal constructor() {
        ...
        internal fun from(cardListResponse: CardListResponse) = apply {
            token = cardListResponse.token
            accountToken = cardListResponse.accountToken
            cardProgramToken = cardListResponse.cardProgramToken
            created = cardListResponse.created
            funding = cardListResponse.funding
            lastFour = cardListResponse.lastFour
            pinStatus = cardListResponse.pinStatus
            spendLimit = cardListResponse.spendLimit
            spendLimitDuration = cardListResponse.spendLimitDuration
            state = cardListResponse.state
            type = cardListResponse.type
            authRuleTokens = cardListResponse.authRuleTokens.map { it.toMutableList() }
            cardholderCurrency = cardListResponse.cardholderCurrency
            digitalCardArtToken = cardListResponse.digitalCardArtToken
            expMonth = cardListResponse.expMonth
            expYear = cardListResponse.expYear
            hostname = cardListResponse.hostname
            memo = cardListResponse.memo
            pendingCommands = cardListResponse.pendingCommands.map { it.toMutableList() }
            productId = cardListResponse.productId
            replacementFor = cardListResponse.replacementFor
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return /* spotless:off */ other is CardListResponse && token == other.token && accountToken == other.accountToken && cardProgramToken == other.cardProgramToken && created == other.created && funding == other.funding && lastFour == other.lastFour && pinStatus == other.pinStatus && spendLimit == other.spendLimit && spendLimitDuration == other.spendLimitDuration && state == other.state && type == other.type && authRuleTokens == other.authRuleTokens && cardholderCurrency == other.cardholderCurrency && digitalCardArtToken == other.digitalCardArtToken && expMonth == other.expMonth && expYear == other.expYear && hostname == other.hostname && memo == other.memo && pendingCommands == other.pendingCommands && productId == other.productId && replacementFor == other.replacementFor && additionalProperties == other.additionalProperties /* spotless:on */
    }
    ...
    override fun toString() =
        "CardListResponse{token=$token, accountToken=$accountToken, cardProgramToken=$cardProgramToken, created=$created, funding=$funding, lastFour=$lastFour, pinStatus=$pinStatus, spendLimit=$spendLimit, spendLimitDuration=$spendLimitDuration, state=$state, type=$type, authRuleTokens=$authRuleTokens, cardholderCurrency=$cardholderCurrency, digitalCardArtToken=$digitalCardArtToken, expMonth=$expMonth, expYear=$expYear, hostname=$hostname, memo=$memo, pendingCommands=$pendingCommands, productId=$productId, replacementFor=$replacementFor, additionalProperties=$additionalProperties}"
}
