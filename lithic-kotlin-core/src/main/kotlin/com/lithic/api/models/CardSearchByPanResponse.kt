...
private constructor(
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [CardSearchByPanResponse].
         *
    ...
    /** A builder for [CardSearchByPanResponse]. */
    class Builder internal constructor() {
        ...
        internal fun from(cardSearchByPanResponse: CardSearchByPanResponse) = apply {
            token = cardSearchByPanResponse.token
            accountToken = cardSearchByPanResponse.accountToken
            cardProgramToken = cardSearchByPanResponse.cardProgramToken
            created = cardSearchByPanResponse.created
            funding = cardSearchByPanResponse.funding
            lastFour = cardSearchByPanResponse.lastFour
            pinStatus = cardSearchByPanResponse.pinStatus
            spendLimit = cardSearchByPanResponse.spendLimit
            spendLimitDuration = cardSearchByPanResponse.spendLimitDuration
            state = cardSearchByPanResponse.state
            type = cardSearchByPanResponse.type
            authRuleTokens = cardSearchByPanResponse.authRuleTokens.map { it.toMutableList() }
            cardholderCurrency = cardSearchByPanResponse.cardholderCurrency
            cvv = cardSearchByPanResponse.cvv
            digitalCardArtToken = cardSearchByPanResponse.digitalCardArtToken
            expMonth = cardSearchByPanResponse.expMonth
            expYear = cardSearchByPanResponse.expYear
            hostname = cardSearchByPanResponse.hostname
            memo = cardSearchByPanResponse.memo
            pan = cardSearchByPanResponse.pan
            pendingCommands = cardSearchByPanResponse.pendingCommands.map { it.toMutableList() }
            productId = cardSearchByPanResponse.productId
            replacementFor = cardSearchByPanResponse.replacementFor
            additionalProperties = cardSearchByPanResponse.additionalProperties.toMutableMap()
        }
        ...
        /**
         * Returns an immutable instance of [CardSearchByPanResponse].
         *
         ...
         */
        fun build(): CardSearchByPanResponse =
    ...
    fun validate(): CardSearchByPanResponse = apply {
        if (validated) {
            return@apply
        }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return /* spotless:off */ other is CardSearchByPanResponse && token == other.token && accountToken == other.accountToken && cardProgramToken == other.cardProgramToken && created == other.created && funding == other.funding && lastFour == other.lastFour && pinStatus == other.pinStatus && spendLimit == other.spendLimit && spendLimitDuration == other.spendLimitDuration && state == other.state && type == other.type && authRuleTokens == other.authRuleTokens && cardholderCurrency == other.cardholderCurrency && cvv == other.cvv && digitalCardArtToken == other.digitalCardArtToken && expMonth == other.expMonth && expYear == other.expYear && hostname == other.hostname && memo == other.memo && pan == other.pan && pendingCommands == other.pendingCommands && productId == other.productId && replacementFor == other.replacementFor && additionalProperties == other.additionalProperties /* spotless:on */
    }
    ...
    override fun toString() =
        "CardSearchByPanResponse{token=$token, accountToken=$accountToken, cardProgramToken=$cardProgramToken, created=$created, funding=$funding, lastFour=$lastFour, pinStatus=$pinStatus, spendLimit=$spendLimit, spendLimitDuration=$spendLimitDuration, state=$state, type=$type, authRuleTokens=$authRuleTokens, cardholderCurrency=$cardholderCurrency, cvv=$cvv, digitalCardArtToken=$digitalCardArtToken, expMonth=$expMonth, expYear=$expYear, hostname=$hostname, memo=$memo, pan=$pan, pendingCommands=$pendingCommands, productId=$productId, replacementFor=$replacementFor, additionalProperties=$additionalProperties}"
}
