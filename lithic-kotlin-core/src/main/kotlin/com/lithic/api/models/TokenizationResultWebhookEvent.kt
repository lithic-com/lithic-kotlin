...
private constructor(
    ...
    /** A builder for [TokenizationResultWebhookEvent]. */
    class Builder internal constructor() {
        ...
        internal fun from(tokenizationResultWebhookEvent: TokenizationResultWebhookEvent) = apply {
            accountToken = tokenizationResultWebhookEvent.accountToken
            cardToken = tokenizationResultWebhookEvent.cardToken
            created = tokenizationResultWebhookEvent.created
            eventType = tokenizationResultWebhookEvent.eventType
            tokenizationResultDetails = tokenizationResultWebhookEvent.tokenizationResultDetails
            tokenizationToken = tokenizationResultWebhookEvent.tokenizationToken
            additionalProperties =
                tokenizationResultWebhookEvent.additionalProperties.toMutableMap()
        }
        ...
        /**
         * Returns an immutable instance of [TokenizationResultWebhookEvent].
         *
    ...
    fun validate(): TokenizationResultWebhookEvent = apply {
        if (validated) {
            return@apply
        }
    ...
    internal fun validity(): Int =
        (if (accountToken.asKnown() == null) 0 else 1) +
            ...
            (tokenizationResultDetails.asKnown()?.validity() ?: 0) +
    ...
    private constructor(
         ...
         */
        fun customerDecision(): String? = customerDecision.getNullable("customer_decision")
         ...
         *   server responded with an unexpected value).
         */
        fun tokenActivatedDateTime(): OffsetDateTime? =
            tokenActivatedDateTime.getNullable("token_activated_date_time")
         ...
         *   server responded with an unexpected value).
         */
        fun walletDecision(): String? = walletDecision.getNullable("wallet_decision")
         ...
         */
        internal fun validity(): Int =
            (if (issuerDecision.asKnown() == null) 0 else 1) +
                (tokenizationDeclineReasons.asKnown()?.sumOf { it.validity().toInt() } ?: 0) +
                (if (customerDecision.asKnown() == null) 0 else 1) +
                (ruleResults.asKnown()?.sumOf { it.validity().toInt() } ?: 0) +
                (if (tokenActivatedDateTime.asKnown() == null) 0 else 1) +
                (tokenizationTfaReasons.asKnown()?.sumOf { it.validity().toInt() } ?: 0) +
                (if (walletDecision.asKnown() == null) 0 else 1)
        ...
        private constructor(private val value: JsonField<String>) : Enum {
            ...
            companion object {
                ...
                fun of(value: String) = TokenizationDeclineReason(JsonField.of(value))
            }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is TokenizationResultWebhookEvent &&
    ...
    override fun toString() =
        "TokenizationResultWebhookEvent{accountToken=$accountToken, cardToken=$cardToken, created=$created, eventType=$eventType, tokenizationResultDetails=$tokenizationResultDetails, tokenizationToken=$tokenizationToken, additionalProperties=$additionalProperties}"
}
