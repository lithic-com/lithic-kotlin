...
private constructor(
     ...
     *   server responded with an unexpected value).
     */
    fun submitted(): OffsetDateTime? = submitted.getNullable("submitted")
    ...
    internal fun validity(): Int =
        (if (token.asKnown() == null) 0 else 1) +
            ...
            (disputedTransactions.asKnown()?.sumOf { it.validity().toInt() } ?: 0) +
            (eventType.asKnown()?.validity() ?: 0) +
            (outstandingRequirements.asKnown()?.sumOf { it.validity().toInt() } ?: 0) +
            (reason.asKnown()?.validity() ?: 0) +
            (status.asKnown()?.validity() ?: 0) +
            (if (submitted.asKnown() == null) 0 else 1) +
    ...
    private constructor(
         ...
         */
        internal fun validity(): Int =
            (eventTokens.asKnown()?.size ?: 0) + (if (transactionToken.asKnown() == null) 0 else 1)
    ...
    private constructor(private val value: JsonField<String>) : Enum {
        ...
        companion object {
            ...
            val QUESTIONNAIRE = of("QUESTIONNAIRE")

            val DOCUMENTS = of("DOCUMENTS")

            fun of(value: String) = OutstandingRequirement(JsonField.of(value))
        }
    ...
    class Reason @JsonCreator private constructor(private val value: JsonField<String>) : Enum {
        ...
        companion object {
            ...
            val CARD_NOT_PRESENT = of("CARD_NOT_PRESENT")

            val CARD_LOST = of("CARD_LOST")

            val CARD_STOLEN = of("CARD_STOLEN")

            val CARD_NEVER_RECEIVED = of("CARD_NEVER_RECEIVED")

            val COUNTERFEIT = of("COUNTERFEIT")

            val ACCOUNT_TAKEOVER = of("ACCOUNT_TAKEOVER")

            val PRODUCT_NOT_RECEIVED = of("PRODUCT_NOT_RECEIVED")

            val NOT_AS_DESCRIBED = of("NOT_AS_DESCRIBED")

            val CREDIT_NOT_PROCESSED = of("CREDIT_NOT_PROCESSED")

            val CANCELLED_RECURRING = of("CANCELLED_RECURRING")

            val PAID_BY_OTHER_MEANS = of("PAID_BY_OTHER_MEANS")

            val DUPLICATE_CHARGE = of("DUPLICATE_CHARGE")

            val LATE_PRESENTMENT = of("LATE_PRESENTMENT")

            val INCORRECT_TRANSACTION_CODE = of("INCORRECT_TRANSACTION_CODE")

            val NO_AUTHORIZATION = of("NO_AUTHORIZATION")

            val DECLINED = of("DECLINED")

            val INCORRECT_AMOUNT = of("INCORRECT_AMOUNT")

            val ATM_CASH_NOT_DISPENSED = of("ATM_CASH_NOT_DISPENSED")

            val ATM_DEPOSIT_WRONG_AMOUNT = of("ATM_DEPOSIT_WRONG_AMOUNT")

            val ATM_DEPOSIT_MISSING = of("ATM_DEPOSIT_MISSING")
    ...
    class Status @JsonCreator private constructor(private val value: JsonField<String>) : Enum {
        ...
        companion object {
            ...
            val INITIALIZING = of("INITIALIZING")

            val AWAITING_INFO = of("AWAITING_INFO")
