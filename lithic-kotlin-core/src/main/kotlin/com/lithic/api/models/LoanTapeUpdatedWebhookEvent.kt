...
private constructor(
     ...
     */
    fun interestDetails(): LoanTape.InterestDetails? =
        interestDetails.getNullable("interest_details")
     ...
     *   server responded with an unexpected value).
     */
    fun dayOfPeriod(): Long? = dayOfPeriod.getNullable("day_of_period")
    ...
    /** A builder for [LoanTapeUpdatedWebhookEvent]. */
    class Builder internal constructor() {
        ...
        internal fun from(loanTapeUpdatedWebhookEvent: LoanTapeUpdatedWebhookEvent) = apply {
            token = loanTapeUpdatedWebhookEvent.token
            accountStanding = loanTapeUpdatedWebhookEvent.accountStanding
            availableCredit = loanTapeUpdatedWebhookEvent.availableCredit
            balances = loanTapeUpdatedWebhookEvent.balances
            created = loanTapeUpdatedWebhookEvent.created
            creditLimit = loanTapeUpdatedWebhookEvent.creditLimit
            creditProductToken = loanTapeUpdatedWebhookEvent.creditProductToken
            date = loanTapeUpdatedWebhookEvent.date
            dayTotals = loanTapeUpdatedWebhookEvent.dayTotals
            endingBalance = loanTapeUpdatedWebhookEvent.endingBalance
            excessCredits = loanTapeUpdatedWebhookEvent.excessCredits
            financialAccountToken = loanTapeUpdatedWebhookEvent.financialAccountToken
            ...
            updated = loanTapeUpdatedWebhookEvent.updated
            version = loanTapeUpdatedWebhookEvent.version
            ytdTotals = loanTapeUpdatedWebhookEvent.ytdTotals
            dayOfPeriod = loanTapeUpdatedWebhookEvent.dayOfPeriod
            tier = loanTapeUpdatedWebhookEvent.tier
            eventType = loanTapeUpdatedWebhookEvent.eventType
            additionalProperties = loanTapeUpdatedWebhookEvent.additionalProperties.toMutableMap()
        }
     ...
     *   expected type.
     */
    fun validate(): LoanTapeUpdatedWebhookEvent = apply {
        if (validated) {
            return@apply
        }
    ...
    internal fun validity(): Int =
        (if (token.asKnown() == null) 0 else 1) +
            ...
            (dayTotals.asKnown()?.validity() ?: 0) +
            (if (endingBalance.asKnown() == null) 0 else 1) +
            (if (excessCredits.asKnown() == null) 0 else 1) +
            ...
            (if (dayOfPeriod.asKnown() == null) 0 else 1) +
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is LoanTapeUpdatedWebhookEvent &&
            token == other.token &&
